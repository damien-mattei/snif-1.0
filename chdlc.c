...
/*
 * Function: DecodeChdlcPkt(struct pcap_pkthdr*, u_int8_t*)
 *
 * Purpose: Decodes Cisco HDLC encapsulated packets
 *
 * Arguments:
...
void DecodeChdlcPkt(struct pcap_pkthdr *pkthdr, u_int8_t *pkt)
{

  if (dump_link_layer) { 
    
    printf( "Cisco HDLC Packet!\n");
    
  }

    if(pkthdr->caplen < CHDLC_HEADER_LEN)
    {
      
            printf("Captured data length < CHDLC header length (%d bytes)\n", pkthdr->caplen);
              return;
    }
    ...
    {
        DecodeIP(pkt + CHDLC_HEADER_LEN,
                 pkthdr->caplen - CHDLC_HEADER_LEN);
    } else {
